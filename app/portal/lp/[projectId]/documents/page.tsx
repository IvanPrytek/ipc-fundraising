...
export default function LPDocumentsPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      const data = await getDocumentFolders(projectId, true);
      setFolders(data);
      setLoading(false);
    }
    load();
  ...
  return <DocumentBrowser folders={folders} projectId={projectId} editable={false} onFoldersChange={setFolders} />;
}
